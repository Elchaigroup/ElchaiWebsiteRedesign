...
const COPY: Record<"EN" | "AR" | "IT", Copy> = {
  EN: { home: "Home", blog: "Blog", back: "Back to all articles", talk: "Talk to us", recent: "Recent Blogs", keepReading: "Keep Reading" },
  AR: { home: "الرئيسية", blog: "المدونة", back: "العودة إلى جميع المقالات", talk: "تحدث معنا", recent: "أحدث المقالات", keepReading: "تابع القراءة" },
  IT: { home: "Home", blog: "Blog", back: "Torna a tutti gli articoli", talk: "Parla con noi", recent: "Articoli recenti", keepReading: "Continua a leggere" },
};

type Post = Pick<BlogPost, "slug" | "href" | "title" | "category" | "date" | "image"> & { body: readonly string[] };

export function BlogPostBody({ post, related }: { post: Post; related: readonly BlogPost[] }) {
  ...
  return (
    <main className="relative" style={{ zIndex: 1 }}>
      <article>
          ...
          <div className="section-box mx-auto max-w-[1100px] px-6 sm:px-12 lg:px-20 py-12 lg:py-16">
            <Reveal>
              ...
              >
                ...
                <Link href="/blog-list" className="hover:text-brand-sky transition-colors">{t.blog}</Link>
                <span aria-hidden="true" className="text-white/30">/</span>
            ...
            <Reveal delay={0.22}>
              <div className="mt-6 flex items-center gap-3">
                <span className="relative w-9 h-9 rounded-full overflow-hidden border border-white/10 shrink-0">
                  <Image src="/elchai/blog/author-avatar.jpg" alt="" aria-hidden="true" fill sizes="36px" className="object-cover" />
                </span>
        ...
        <section className="relative pb-16 lg:pb-24" aria-label={t.blog}>
          <div className="section-box mx-auto max-w-[820px] px-6 sm:px-10 lg:px-12 py-8 lg:py-12">
            <div className="flex flex-col gap-7">
              {post.body.map((para, i) => (
                <Reveal key={i} delay={0.06 + (i % 4) * 0.04}>
                  {para.length < 220 && i > 0 ? (
                    <div className="relative rounded-2xl border-s-2 border-brand-sky/60 bg-[rgba(82,184,255,0.05)] ps-6 pe-6 py-5">
            ...
            <Reveal delay={0.20}>
              <div className="mt-14 flex items-center justify-between gap-4 border-t border-white/10 pt-8">
                <Link
                  href="/blog-list"
                  className="inline-flex items-center gap-2 text-[13.5px] text-white/85 hover:text-brand-sky transition-colors"
                >
      ...
      {related.length > 0 && (
        <section className="relative py-10 lg:py-14" aria-label={t.recent}>
